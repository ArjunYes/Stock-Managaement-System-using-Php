...
<html>
<head>
   <title>SUPPLY BACK TO OFFICE</title>
...
<form method="POST" >
        ...
        <select class="in1" name="selectbox1" required="">
            <option  value="">Select Product </option>
                ...
                while ($row = mysqli_fetch_assoc($result)){
                    ...
                    $NUM = $row['No_of_units'];
                ?>
                <option value="<?php echo $IC ?>"><?php echo $IN," - ",$IC ?></option>
                <?php
            }
            ?>
        </select>
        ...
        <input class="btn1" type="submit" name="sub1" value="SEND BACK TO OFFICE">
    ...
    if(isset($_POST['sub1'])){
        ...
        if(mysqli_num_rows($result)==0){ #no stock with given hospital details
          echo "<script> alert('No units found in hospital'); window.location.href = 'Staff_home.php' </script>";  
            ...
            if($units_at_hospital<$NOI){
                echo "<script> alert('Hospital dosnt have eough units to supply ');  window.location.href = 'Staff_home.php' </script>";
                exit();
            ...
            $sql6 = "update hospital_stock SET No_of_units = '$new_units_at_hospital' where Item_code ='$IC' and Hospital_id = '$HID'  ";
             if(mysqli_query($conn,$sql6)){
            echo "Record updated succesfully";

             echo "<script> alert('Deletion succesfull');  window.location.href = 'Staff_home.php' </script>";
            //code
            }else{
